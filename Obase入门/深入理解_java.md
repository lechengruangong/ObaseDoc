在之前的[快速开始](./快速开始_java.md)和[最佳实践](./最佳实践_java.md)里,我们已经了解了Obase是如何通过数据模型类操作数据库的.
...
- 属性,一般而言是将类上的普通([Obase基元类型](../Obase基础知识/Obase的基元类型与默认数据库类型映射_java.md)或者结构体)属性配置为的Obase对象数据模类型,属性在数据源中一般存储于某张具体表的字段中.
  Obase将属性分为两种,类型为[Obase基元类型](../Obase基础知识/Obase的基元类型与默认数据库类型映射_java.md)的,称之为普通类型.
  类型为类,但类上的[Obase基元类型](../Obase基础知识/Obase的基元类型与默认数据库类型映射_java.md)想要存储于某个数据源的表字段的,称之为复杂类型.
...
- 普通属性,类型为[Obase基元类型](../Obase基础知识/Obase的基元类型与默认数据库类型映射_java.md)的在实体型或者显示关联型上定义的属性称之为普通属性.
  普通属性需要在Obase的对象数据模型中配置为属性,一般情况下不需要特别的配置,使用默认的配置即可.
- 复杂属性,某个实体型或者关联型的属性定义为某个类,但此类的属性想要和此实体型或关联型一并存储,此时会将此类型的属性配置为复杂属性.
...
更多的概念,可以阅读[Obase的对象数据模型基础配置概念](../Obase基础知识/Obase的对象数据模型基础配置概念_java.md).

当然,每个部分实际配置时各自需要根据具体的情景进行配置,除了这三个部分之外,Obase还有一些功能性的配置如对象更改通知,并发策略之类的配置,这些可以参考Obase基础知识中的具体文章.

而基础的实体和关系的配置,请阅读[如何配置对象数据模型](./Obase如何配置对象数据模型_java.md)来深入了解配置过程.