...
在dotNet中Obase基元类型为内置基元类型+基元类型的Nullable<>包装结构+string(字符串)类型+decimal(精确十进制数)+decimal的Nullable<>包装结构+datetime(日期时间)+datetime的Nullable<>包装结构+timespan(时间) +timespan的Nullable<>包装结构+guid(6.1新增)+GUID的Nullable<>包装结构+枚举.
