...
最后,Obase的对象数据模型的建造支持扩展,详细内容可以参考[Obase如何扩展建模管道](../Obase进阶使用/Obase如何扩展建模管道_dotNet.md),此下内容会提到在默认管道情况下指的都是当前建模管道中只有默认管道节点的情形.
...
13. 处理完整性检查
    ...
    2. 更多的完整性检查相关内容,请参考[Obase的完整性检查与错误速查](./Obase的对象数据模型完整性检查与错误速查_dotNet.md).
14. 处理结构映射
    1. 如果启用了结构映射,则进行结构映射.
    2. 更多的结构映射相关内容,请参考[Obase如何配置结构映射](../Obase进阶使用/Obase如何配置结构映射_dotNet.md).