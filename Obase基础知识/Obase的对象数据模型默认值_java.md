...
在总结以往的领域模型的配置逻辑后,Obase加入了大量的推断,用以辅助进行Obase的反射建模配置.
...
结构化类型的推断发生在建模中的两个过程里,一个是在调用modelBuilder的registerType(String assembly)方法时,此处会将符合推断的类型注册为结构化类型,另外一个是在对所有已配置的结构化类型进行配置时,会将符合推断的配置项进行配置.
...
类型元素推断发生在已经根据结构化类型配置创建结构化类型后,对该类型的属性(这里的属性指定是使用JAVA内省机制获取的属性)进行反射处理时.
...
将属性类型为Obase的基元类型的属性(public,无访问限定符)推断为属性.
...
将关联型(显式或隐式)属性类型是已注册的实体型的属性(public,无访问限定符)推断为关联端.
...
将实体型属性类型是已注册的实体型或关联型(显式或隐式)的属性(public,无访问限定符)推断为关联引用.
