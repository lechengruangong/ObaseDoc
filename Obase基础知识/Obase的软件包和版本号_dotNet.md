...
在此处记录了从6.4之后的每个次版本号升级时,哪些改动导致与之前API不兼容的速览.

- 6.4->6.5版本:主要为继承配置的改动,在6.5中配置具体类型判别器的方法进行了优化,加入了没有配置具体类型判别器时使用默认的具体类型判别器的逻辑,导致与6.4的配置不兼容.
  
    具体的配置区别可以查看[Obase如何配置继承关系](../Obase进阶使用/Obase如何配置继承关系_dotNet.md)中的详细内容.