...
这四条管道分别在建模过程的不同阶段被调用,具体请参考[Obase的对象数据模型建模过程](../Obase基础知识/Obase的对象数据模型建模过程_dotNet.md)文档,其中第7,8,10,12步中使用的管道就是对应的管道.
...
///     映射表前缀补充配置器
/// </summary>
public class PrefixComplementConfigurator : IComplementConfigurator
{
    /// <summary>
    ///     初始化映射表前缀补充配置器
    /// </summary>
    /// <param name="next">下一个补充配置器</param>
    public PrefixComplementConfigurator(IComplementConfigurator next)
    {
        Next = next;
    }

    /// <summary>根据类型配置项中的元数据配置指定的类型。</summary>
    ...
    public void Configurate(StructuralType targetType, StructuralTypeConfiguration configuration)
    {
        ...
        {
            objectType.TargetTable = $"Enc_{objectType.TargetTable}";
        }
    }

    /// <summary>补充配置管道中的下一个配置器。</summary>
...
{
    //向管道中注册新的节点
    modelBuilder.Use(p => new PrefixComplementConfigurator(p));
