...
但仅对属性配置序列化器时,如果遇到较为复杂的情况(如某些对象属性要特殊存储,对象间存在互相引用等)时,就需要定义一个继承自TextSerializer的类型,并且在实现的方法中针对不同的类型书写具体的序列化方案.
...
假设我们有一个Service服务类,上面引用了Identity身份和Component组件类,这三个都需要序列化为Json字符串进行存储.
...
```
这里的配置需要注意的是,一个序列化对象的配置分为三种配置属性,引用和构造函数.
