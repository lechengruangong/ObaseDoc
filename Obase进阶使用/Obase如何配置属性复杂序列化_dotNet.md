...
在引入了SOdm之后,我们除了可以将一般的类型序列化之外,还可以将定义类型为接口或者抽象类的属性进行序列化了,只需要将接口的具体实现类型或者抽象类的具体子类注册为序列化实体型就可以配置每个具体类型的序列化配置,这些定义类型是接口或抽象类的属性在使用时的对象一定是具体的类型,所以只要配置具体类型的序列化模型.
