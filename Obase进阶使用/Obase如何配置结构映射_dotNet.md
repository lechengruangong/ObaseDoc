...
在上下文配置提供器中,重写protected bool EnableStructMapping { get; }属性访问器,当此属性的返回值设置为true时,在建造对象数据模型之后就会执行结构映射.
...
对于decimal类型,使用HasPrecision可以指定字段的精度,设置的值对应字段中小数部分的位数,应当在0-28之间.

下表为默认情况下,十进制数(decimal)类型的字段精度:
