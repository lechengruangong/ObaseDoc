...
### 特定配置写法
1. [Obase如何配置两个类之间的多个不同隐式关系(dotNet版)](./Obase如何配置两个类之间的多个不同隐式关系_dotNet.md)和[Obase如何配置两个类之间的多个不同隐式关系(java版)](./Obase如何配置两个类之间的多个不同隐式关系_java.md).主要介绍了为两个类之间的多个不同关系进行配置的示例,比如员工和房间之间可以存在两种关系,一种是员工工作所在的房间,一种是员工管理的房间这种情况的配置示例.
